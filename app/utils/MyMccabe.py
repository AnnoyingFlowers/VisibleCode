    ...
    if options.dot:
        ...
        for graph in visitor.graphs.values():
            if graph.complexity() >= options.threshold:
                info[graph.entity] = graph.complexity()
                # print(graph.name, graph.complexity())
    return info


def mccabe(src, threshold=5):
    current_address = os.path.abspath(src)
    complex_num = 0  # 复杂度 > threshold的方法个数
    complex_sum = 0  # 复杂度之和
    complex_res = {}  # 复杂度字典
    for parent, dirnames, filenames in os.walk(current_address):
        ...
        for filename in filenames:
            if os.path.splitext(filename)[1] in ['.py', '.ipynb']:
                ...
                info = main(['--min', str(threshold), dir])
                complex_res[parent][filename] = {}
                for k, v in info.items():
                    complex_res[parent][filename][k] = v
                    complex_num += 1
                    complex_sum += v
    # print('复杂度大于{}的方法个数 : {}'.format(threshold, complex_num))
    # print('复杂度大于{}的复杂度之和 : {}'.format(threshold, complex_sum))
    return complex_res, complex_num, complex_sum
